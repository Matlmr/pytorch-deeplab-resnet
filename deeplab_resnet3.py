...
# Adding the Pyramid Pooling Module
class PSP(nn.Module):
    def __init__(self,block,NoLabels):
        super(PSP,self).__init__()
        self.FeatureMap = ResNet(block,NoLabels)
    ...
    def forward(self,x):
        ...
        x1 = self.conv1(F.avg_pool2d(x,input_size))
        x2 = self.conv1(F.avg_pool2d(x,int(input_size/2)))
        x3 = self.conv1(F.avg_pool2d(x,int(input_size/3)))
        x4 = self.conv1(F.avg_pool2d(x,int(input_size/6)))
        x.append(F.upsample_bilinear(x1, input_size))
        x.append(F.upsample_bilinear(x2, input_size))
        x.append(F.upsample_bilinear(x3, input_size))
        x.append(F.upsample_bilinear(x4, input_size))
